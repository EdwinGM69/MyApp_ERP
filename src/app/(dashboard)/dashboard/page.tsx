      ...
      <div className="flex-1 overflow-y-auto p-8 bg-slate-50 dark:bg-background-dark">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <StatsCard
            ...
            value={loading ? '...' : formatCurrency(data?.inventarioValue ?? 0, { symbol: monedaSimbolo })}
            icon="inventory"
          ...
          <StatsCard
            ...
            value={loading ? '...' : formatCurrency(data?.ventasMes?.total ?? 0, { symbol: monedaSimbolo })}
            icon="payments"
          ...
          <StatsCard
            ...
            value={loading ? '...' : formatCurrency(data?.ventasDia?.total ?? 0, { symbol: monedaSimbolo })}
            icon="today"
              ...
              {semestre.map((item, i) => {
                ...
                return (
                  <div key={i} className="flex-1 flex flex-col items-center gap-2">
                    ...
                    >
                      <div className="absolute -top-8 left-1/2 -translate-x-1/2 bg-slate-800 text-white text-xs py-1 px-2 rounded opacity-0 group-hover:opacity-100 whitespace-nowrap pointer-events-none z-10 transition-opacity">
                        {mounted ? formatCurrency(Number(item.total), { symbol: monedaSimbolo }) : '...'}
                      </div>
                    </div>
                ...
                data.topProductos.map((item, i) => (
                  <div key={i} className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-slate-500">{formatCurrency(Number(item.total), { symbol: monedaSimbolo })}</p>
                    </div>
                  ...
                  data.ventasRecientes.map((v) => (
                    <tr key={v.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                      ...
                      <td className="px-6 py-4 text-sm font-semibold text-right text-slate-900 dark:text-white">
                        {formatCurrency(Number(v.total), { symbol: monedaSimbolo })}
                      </td>
                    </tr>
                  ))
                ) : (
