...
export default function MediosPagoPage() {
  ...
  const [selected, setSelected] = useState<MedioPago | null>(null)
  ...
  const fetchList = useCallback(async () => {
    ...
    try {
      const res = await apiFetch(`/api/tesoreria/medios-pago?search=${search}`)
  ...
  useEffect(() => {
    if (selectedId) {
      setSelected(medios.find(m => m.id === selectedId) || null)
    }
  }, [selectedId, medios])

  const handleOpenEditor = (m?: any) => {
    setEditingData(m || {
      descripcion: '',
      aplica_denominacion: false,
      activo: true
    })
    setIsEditing(true)
  }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (res.ok) {
        toast.success(isNew ? 'Medio de pago creado' : 'Medio de pago actualizado')
        setIsEditing(false)
        fetchList()
      } else {
        const json = await res.json()
  ...
  return (
    <div className="flex flex-col flex-1 h-full overflow-hidden bg-white dark:bg-slate-950">
      <Topbar title="Definición de Medios de Pago" />
              ...
              >
                <div className="flex items-center gap-3">
                  <div className={cn(
                    ...
                    selectedId === m.id ? "bg-primary text-white" : "bg-slate-100 dark:bg-slate-800 text-slate-400"
                  )}>
                    {m.descripcion.slice(0, 2).toUpperCase()}
                  </div>
                  <div className="min-w-0 flex-1">
                    <span className={cn(
                      "text-sm font-bold block truncate tracking-tight transition-colors",
                      selectedId === m.id ? "text-slate-900 dark:text-white" : "text-slate-600 dark:text-slate-400"
                    )}>
                      {m.descripcion}
                    </span>
                    <span className="text-[10px] font-medium text-slate-400 uppercase tracking-tighter">
                      TESORERÍA · MEDIO PAGO
                    </span>
                  </div>
                </div>
              </button>
          ...
          {isEditing && editingData ? (
              ...
              <div className="flex items-center justify-between mb-6 pb-6 border-b border-slate-100 dark:border-slate-800">
                <div>
                  <h2 className="text-3xl font-black text-slate-900 dark:text-white tracking-tighter leading-none uppercase mb-2">
                    {editingData.id ? 'Actualizar Medio de Pago' : 'Nuevo Medio de Pago'}
                  </h2>
                  <p className="text-slate-400 text-xs font-medium uppercase tracking-widest">Configuración de Medio de Pago en Tesorería</p>
                </div>
                <button
              ...
              <div className="space-y-6">
                <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Descripción del Medio de Pago</label>
                    <input
                      type="text"
                ...
                <div className="bg-slate-50/50 dark:bg-slate-900/50 p-6 rounded-3xl border border-slate-100 dark:border-slate-800">
                  <div className="space-y-2">
                    ...
                    <div className="h-10 flex items-center">
                      <Switch
                        checked={editingData.aplica_denominacion}
                        onChange={(c) => setEditingData({ ...editingData, aplica_denominacion: c })}
                      />
                      <span className="ml-3 text-[10px] font-bold text-slate-500 uppercase tracking-tight">Habilitar desglose de billetes/monedas para este medio</span>
                    </div>
                  </div>
                </div>
              ...
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <p className="text-slate-400 text-sm font-medium tracking-tight">
                    Definición de Medio de Pago para Operaciones de Tesorería
                  </p>
                </div>
                  ...
                  <div className="grid grid-cols-1 gap-4">
                    <div className="p-6 bg-slate-50/50 dark:bg-slate-900/50 rounded-3xl border border-slate-100 dark:border-slate-800 text-center">
                      ...
                      <div className={cn(
                        ...
                        selected.aplica_denominacion ? "bg-emerald-500/10 text-emerald-600" : "bg-slate-100 text-slate-400"
                      )}>
                        <span className="material-symbols-outlined">{selected.aplica_denominacion ? 'check_circle' : 'cancel'}</span>
                      </div>
                      <h4 className="text-sm font-black text-slate-800 dark:text-white mb-1 uppercase tracking-tight">
                        {selected.aplica_denominacion ? 'Habilitado' : 'Deshabilitado'}
                      </h4>
                      <p className="text-[10px] text-slate-500 font-medium uppercase tracking-tighter">Aplica desglose por denominación</p>
                    </div>
                  </div>
                </div>
            ...
            <div className="h-full flex flex-col items-center justify-center text-slate-300 gap-6 animate-in fade-in duration-700">
              ...
              <div className="text-center">
                <p className="text-sm font-black uppercase tracking-[0.3em] opacity-30 leading-none mb-2">Medios de Pago</p>
                <p className="text-xs font-medium text-slate-400 italic">Selecciona un medio de pago para visualizar sus configuraciones.</p>
              </div>
            </div>
          )}
        </div>
