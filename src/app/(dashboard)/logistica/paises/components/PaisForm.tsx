...
export default function PaisForm({ paisToEdit }: PaisFormProps) {
  ...
  const [prefijoTelefonico, setPrefijoTelefonico] = useState('')
  const [activo, setActivo] = useState(true)

  useEffect(() => {
    if (paisToEdit) {
      setDescripcion(paisToEdit.descripcion)
      setAbreviatura(paisToEdit.abreviatura)
      setPrefijoTelefonico(paisToEdit.prefijo_telefonico || '')
      setActivo(paisToEdit.activo)
    }
  }, [paisToEdit])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      descripcion,
      abreviatura,
      prefijo_telefonico: prefijoTelefonico || null,
      activo,
    }

    try {
      const res = await apiFetch('/api/logistica/paises', {
        method: paisToEdit ? 'PUT' : 'POST',
      ...
      toast.success(paisToEdit ? 'País actualizado' : 'País creado')
      router.push('/logistica/paises')
      router.refresh()
    } catch (error: any) {
    ...
    <div className="flex flex-col flex-1 bg-slate-50/50">
      ...
      <div className="sticky top-0 z-30 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-200/60 dark:border-slate-800/60 py-4 px-8 flex items-center justify-between shadow-sm tracking-tight transition-all">
        <div className="flex items-center gap-4">
          <button
            type="button"
            onClick={() => router.push('/logistica/paises')}
          ...
          <div>
            ...
            <h1 className="text-xl font-black text-slate-900 dark:text-white tracking-tight leading-none">
              {paisToEdit ? 'Editar País' : 'Registro de País'}
            </h1>
            <p className="text-[11px] text-slate-500 font-medium mt-1">
              Configure los países para la gestión de direcciones y telecomunicaciones.
            </p>
          </div>
        </div>
      ...
      <div className="max-w-[1200px] mx-auto w-full px-8 py-6">
        <form id="pais-form" onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {/* Main Column */}
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm p-8">
              <div className="space-y-6">
                <div>
                  <label className="block text-xs font-black text-slate-800 dark:text-white uppercase tracking-wider mb-4">Nombre del País</label>
                  <input
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <label className="block text-xs font-black text-slate-800 dark:text-white uppercase tracking-wider mb-4">Abreviatura / ISO</label>
                    <input
                  ...
                  <div>
                    <label className="block text-xs font-black text-slate-800 dark:text-white uppercase tracking-wider mb-4">Prefijo Telefónico</label>
                    <div className="relative">
                      <span className="absolute left-6 top-1/2 -translate-y-1/2 text-slate-400 font-bold">+</span>
                      <input
                        type="text" value={prefijoTelefonico} onChange={e => setPrefijoTelefonico(e.target.value.replace(/\D/g, ''))}
                        className="w-full pl-9 pr-6 py-2 bg-slate-50/50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-xl text-base font-medium outline-none focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all placeholder:text-slate-400 font-mono"
                        placeholder="34"
                      />
                    </div>
                  </div>
            ...
            <div className="bg-blue-50/50 dark:bg-blue-500/5 rounded-2xl p-6 border border-blue-100 dark:border-blue-500/10 flex items-start gap-4">
              ...
              <div>
                <h4 className="text-sm font-black text-slate-900 dark:text-white leading-tight mb-1">Información Geográfica</h4>
                <p className="text-xs text-slate-500 font-medium">Los datos de países son fundamentales para la validación de números telefónicos y direcciones de facturación.</p>
              </div>
            </div>
          </div>
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-black text-slate-800 dark:text-white leading-tight">
                      {paisToEdit?.usuario_creador?.nombre || '--'}
                    </p>
                    <p className="text-[10px] text-slate-400 font-medium mt-0.5">
                      {paisToEdit?.created_at ? format(new Date(paisToEdit.created_at), 'dd MMM, hh:mm aa') : '--'}
                    </p>
                  </div>
                </div>

                <div className="flex items-start gap-4">
                  <div className="size-10 rounded-full bg-slate-10/50 dark:bg-slate-700/50 flex items-center justify-center shrink-0">
                  ...
                  <div>
                    ...
                    {paisToEdit?.usuario_modificador?.nombre ? (
                      <>
                        <p className="text-sm font-black text-slate-800 dark:text-white leading-tight">{paisToEdit.usuario_modificador.nombre}</p>
                        <p className="text-[10px] text-slate-400 font-medium mt-0.5">
                           {paisToEdit.updated_at ? format(new Date(paisToEdit.updated_at), 'dd MMM, hh:mm aa') : ''}
                        </p>
                      </>
                    ) : (
            ...
            <div className="bg-[#0f172a] rounded-2xl p-8 text-white shadow-xl shadow-slate-900/20 group hover:scale-[1.02] transition-all cursor-default overflow-hidden relative">
              ...
              <div className="relative z-10">
                ...
                <p className="text-[11px] text-slate-400 font-medium leading-relaxed">
                  Utilice el formato ISO de 2 letras para las abreviaturas (ej: ES para España, MX para México).
                </p>
              </div>
            </div>
          </div>
