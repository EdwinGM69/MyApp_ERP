...
export default function DocumentoIdentificacionForm({ documentoToEdit }: DocumentoFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await apiFetch('/api/logistica/documentos-identificacion', {
        method: documentoToEdit ? 'PUT' : 'POST',
        headers: {
      ...
      toast.success(documentoToEdit ? 'Documento actualizado' : 'Documento creado')
      router.push('/logistica/documentos-identificacion')
      router.refresh()
    } catch (error: any) {
    ...
    <div className="flex flex-col flex-1 bg-slate-50/50">
      ...
      <div className="sticky top-0 z-30 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-200/60 dark:border-slate-800/60 py-4 px-8 flex items-center justify-between shadow-sm tracking-tight transition-all">
        <div className="flex items-center gap-4">
          <button
            type="button"
            onClick={() => router.push('/logistica/documentos-identificacion')}
          ...
          <div>
            ...
            <h1 className="text-xl font-black text-slate-900 dark:text-white tracking-tight leading-none">
              {documentoToEdit ? 'Editar Documento' : 'Registro de Documento'}
            </h1>
            <p className="text-[11px] text-slate-500 font-medium mt-1">
              Configure los tipos de documentos de identificación aceptados en el sistema.
            </p>
          </div>
        </div>
      ...
      <div className="max-w-[1200px] mx-auto w-full px-8 py-6">
        <form id="documento-form" onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {/* Main Column */}
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm p-8">
              <div className="space-y-6">
                <div>
                  <label className="block text-xs font-black text-slate-800 dark:text-white uppercase tracking-wider mb-4">Descripción del Documento</label>
                  <input
            ...
            <div className="bg-blue-50/50 dark:bg-blue-500/5 rounded-2xl p-6 border border-blue-100 dark:border-blue-500/10 flex items-start gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 font-medium">Asegúrese que la descripción y abreviatura sean claras.</p>
              </div>
            </div>
          </div>
              ...
              <div className="space-y-6">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-black text-slate-800 dark:text-white leading-tight">
                      {documentoToEdit?.usuario_creador?.nombre || '--'}
                    </p>
                    <p className="text-[10px] text-slate-400 font-medium mt-0.5">
                      {documentoToEdit?.created_at ? format(new Date(documentoToEdit.created_at), 'dd MMM, hh:mm aa') : '--'}
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    {documentoToEdit?.usuario_modificador?.nombre ? (
                      <>
            ...
            <div className="bg-[#0f172a] rounded-2xl p-8 text-white shadow-xl shadow-slate-900/20 group hover:scale-[1.02] transition-all cursor-default overflow-hidden relative">
              ...
              <div className="relative z-10">
                ...
                <p className="text-[11px] text-slate-400 font-medium leading-relaxed">
                  Defina los tipos de documentos para validar correctamente la identidad de sus clientes y proveedores.
                </p>
              </div>
            </div>
          </div>
