...
export default function CategoriaForm({ categoriaToEdit }: CategoriaFormProps) {
  ...
  useEffect(() => {
    if (categoriaToEdit) {
      setCodigo(categoriaToEdit.codigo)
      setDescripcion(categoriaToEdit.descripcion)
      setActivo(categoriaToEdit.activo)
    }
  }, [categoriaToEdit])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await apiFetch('/api/materiales/categorias', {
        method: categoriaToEdit ? 'PUT' : 'POST',
        headers: {
      ...
      toast.success(categoriaToEdit ? 'Categoría actualizada' : 'Categoría creada')
      router.push('/maestros/logistica/categorias')
      router.refresh()
    } catch (error: any) {
    ...
    <div className="flex flex-col flex-1 min-h-screen bg-slate-50/50">
      ...
      <div className="sticky top-0 z-30 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-200/60 dark:border-slate-800/60 py-4 px-8 flex items-center justify-between shadow-sm tracking-tight transition-all">
        <div className="flex items-center gap-4">
          <button
            type="button"
            onClick={() => router.push('/maestros/logistica/categorias')}
          ...
          <div>
            ...
            <h1 className="text-xl font-black text-slate-900 dark:text-white tracking-tight leading-none">
              {categoriaToEdit ? 'Editar Categoría' : 'Registro de Categoría'}
            </h1>
            <p className="text-[11px] text-slate-500 font-medium mt-1">
              Gestione las categorías de materiales para su catálogo y almacén.
            </p>
          </div>
        </div>
      ...
      <div className="max-w-[1200px] mx-auto w-full px-8 py-6">
        <form id="categoria-form" onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {/* Main Column */}
          <div className="lg:col-span-2 space-y-6">
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="Ingrese el nombre o descripción de la categoría..."
                  />
                </div>
              </div>
            </div>
            ...
            <div className="bg-blue-50/50 dark:bg-blue-500/5 rounded-2xl p-6 border border-blue-100 dark:border-blue-500/10 flex items-start gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 font-medium">Asegúrese que el código sea descriptivo para facilitar la búsqueda.</p>
              </div>
            </div>
          </div>
              ...
              <div className="space-y-6">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-black text-slate-800 dark:text-white leading-tight">
                      {categoriaToEdit?.usuario_creador?.nombre || '--'}
                    </p>
                    <p className="text-[10px] text-slate-400 font-medium mt-0.5">
                      {categoriaToEdit?.created_at ? format(new Date(categoriaToEdit.created_at), 'dd MMM, hh:mm aa') : '--'}
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    {categoriaToEdit?.usuario_modificador?.nombre ? (
                      <>
                        <p className="text-sm font-black text-slate-800 dark:text-white leading-tight">{categoriaToEdit.usuario_modificador.nombre}</p>
                        <p className="text-[10px] text-slate-400 font-medium mt-0.5">
                          {categoriaToEdit.updated_at ? format(new Date(categoriaToEdit.updated_at), 'dd MMM, hh:mm aa') : ''}
                        </p>
                      </>
                    ) : (
            ...
            <div className="bg-[#0f172a] rounded-2xl p-8 text-white shadow-xl shadow-slate-900/20 group hover:scale-[1.02] transition-all cursor-default overflow-hidden relative">
              ...
              <div className="relative z-10">
                ...
                <p className="text-[11px] text-slate-400 font-medium leading-relaxed">
                  Las categorías permiten organizar su inventario y obtener reportes más detallados por tipo de producto.
                </p>
              </div>
            </div>
          </div>
