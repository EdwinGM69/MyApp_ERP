...
export default function CategoriasPage() {
  ...
  async function handleImportFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setImportTotal(categorias.length)

      const res = await apiFetch('/api/materiales/categorias/import', {
        method: 'POST',
      ...
      if (!res.ok) {
        toast.error(result.error || 'Error al importar categorías.')
        return
      }

      if (result.created > 0) {
        toast.success(`Se crearon ${result.created} categoría(s) correctamente.`)
      }
