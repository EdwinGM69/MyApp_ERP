...
export default function TipoForm({ tipoToEdit }: TipoFormProps) {
  ...
  useEffect(() => {
    if (tipoToEdit) {
      setCodigo(tipoToEdit.codigo)
      setDescripcion(tipoToEdit.descripcion)
      setActivo(tipoToEdit.activo)
    }
  }, [tipoToEdit])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(tipoToEdit ? 'Tipo de material actualizado' : 'Tipo de material creado')
      router.push('/maestros/logistica/tipos-material')
      router.refresh()
    } catch (error: any) {
    ...
    <div className="flex flex-col flex-1 min-h-screen bg-slate-50/50">
      ...
      <div className="sticky top-0 z-30 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-200/60 dark:border-slate-800/60 py-4 px-8 flex items-center justify-between shadow-sm tracking-tight transition-all">
        <div className="flex items-center gap-4">
          <button
            type="button"
            onClick={() => router.push('/maestros/logistica/tipos-material')}
          ...
          <div>
            ...
            <h1 className="text-xl font-black text-slate-900 dark:text-white tracking-tight leading-none">
              {tipoToEdit ? 'Editar Tipo' : 'Registro de Tipo'}
            </h1>
            <p className="text-[11px] text-slate-500 font-medium mt-1">
              Gestione los tipos de materiales para clasificar adecuadamente sus productos.
            </p>
          </div>
        </div>
      ...
      <div className="max-w-[1200px] mx-auto w-full px-8 py-6">
        <form id="tipo-form" onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {/* Main Column */}
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm p-8">
              <div className="space-y-6">
                <div>
                  ...
                  />
                  <p className="mt-2 text-[10px] text-slate-400 font-medium">Identificador único alfanumérico para el tipo de material.</p>
                </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="Ingrese el nombre o descripción del tipo de material..."
                  />
                </div>
              </div>
            </div>
            ...
            <div className="bg-blue-50/50 dark:bg-blue-500/5 rounded-2xl p-6 border border-blue-100 dark:border-blue-500/10 flex items-start gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 font-medium">Asegúrese que el código sea claro para identificar el tipo de material rápidamente.</p>
              </div>
            </div>
          </div>
              ...
              <div className="space-y-6">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-black text-slate-800 dark:text-white leading-tight">
                      {tipoToEdit?.usuario_creador?.nombre || '--'}
                    </p>
                    <p className="text-[10px] text-slate-400 font-medium mt-0.5">
                      {tipoToEdit?.created_at ? format(new Date(tipoToEdit.created_at), 'dd MMM, hh:mm aa') : '--'}
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    {tipoToEdit?.usuario_modificador?.nombre ? (
                      <>
                        <p className="text-sm font-black text-slate-800 dark:text-white leading-tight">{tipoToEdit.usuario_modificador.nombre}</p>
                        <p className="text-[10px] text-slate-400 font-medium mt-0.5">
                          {tipoToEdit.updated_at ? format(new Date(tipoToEdit.updated_at), 'dd MMM, hh:mm aa') : ''}
                        </p>
                      </>
                    ) : (
            ...
            <div className="bg-[#0f172a] rounded-2xl p-8 text-white shadow-xl shadow-slate-900/20 group hover:scale-[1.02] transition-all cursor-default overflow-hidden relative">
              ...
              <div className="relative z-10">
                ...
                <p className="text-[11px] text-slate-400 font-medium leading-relaxed">
                  Los tipos de material ayudan a diferenciar entre insumos, productos finales, repuestos, etc.
                </p>
              </div>
            </div>
          </div>
