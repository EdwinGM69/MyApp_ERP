...
export default function UbicacionForm({ ubicacionToEdit }: UbicacionFormProps) {
  ...
  useEffect(() => {
    if (ubicacionToEdit) {
      setCodigo(ubicacionToEdit.codigo)
      setDescripcion(ubicacionToEdit.descripcion)
      setActivo(ubicacionToEdit.activo)
    }
    setMounted(true)
  }, [ubicacionToEdit])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await apiFetch('/api/logistica/ubicaciones', {
        method: ubicacionToEdit ? 'PUT' : 'POST',
        headers: {
      ...
      toast.success(ubicacionToEdit ? 'Ubicación actualizada' : 'Ubicación creada')
      router.push('/maestros/ubicaciones')
      router.refresh()
    } catch (error: any) {
    ...
    <div className="flex flex-col flex-1 min-h-screen bg-slate-50/50">
      ...
      <div className="sticky top-0 z-30 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-200/60 dark:border-slate-800/60 py-4 px-8 flex items-center justify-between shadow-sm tracking-tight transition-all">
        <div className="flex items-center gap-4">
          <button
            type="button"
            onClick={() => router.push('/maestros/ubicaciones')}
          ...
          <div>
            ...
            <h1 className="text-xl font-black text-slate-900 dark:text-white tracking-tight leading-none">
              {ubicacionToEdit ? 'Editar Ubicación' : 'Registro de Ubicación'}
            </h1>
            <p className="text-[11px] text-slate-500 font-medium mt-1">
              Configure las ubicaciones físicas para organizar sus materiales.
            </p>
          </div>
        </div>
      ...
      <div className="max-w-[1200px] mx-auto w-full px-8 py-6">
        <form id="ubicacion-form" onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {/* Main Column */}
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm p-8">
              <div className="space-y-6">
                <div>
                  <label className="block text-xs font-black text-slate-800 dark:text-white uppercase tracking-wider mb-2">Código de la Ubicación</label>
                  <input
                    required value={codigo} onChange={e => setCodigo(e.target.value)}
                    type="text"
                    className="w-full px-6 py-4 bg-slate-50/50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-2xl text-base font-medium outline-none focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all placeholder:text-slate-400"
                    placeholder="Ej: PAS-01-EST-B"
                  />
                </div>
                <div>
                  <label className="block text-xs font-black text-slate-800 dark:text-white uppercase tracking-wider mb-2">Descripción / Notas</label>
                  <textarea
                    ...
                    placeholder="Ingrese detalles sobre esta ubicación (Ej: Estantería B, tercer nivel)..."
                  />
                </div>
              </div>
            </div>
            ...
            <div className="bg-blue-50/50 dark:bg-blue-500/5 rounded-2xl p-6 border border-blue-100 dark:border-blue-500/10 flex items-start gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 font-medium">Las ubicaciones permiten segmentar sus almacenes para una búsqueda rápida de materiales.</p>
              </div>
            </div>
          </div>
              ...
              <div className="space-y-6">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-black text-slate-800 dark:text-white leading-tight">
                      {ubicacionToEdit?.usuario_creador?.nombre || '--'}
                    </p>
                    <p className="text-[10px] text-slate-400 font-medium mt-0.5">
                      {mounted && ubicacionToEdit?.created_at ? format(new Date(ubicacionToEdit.created_at), 'dd MMM, hh:mm aa') : '--'}
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    {ubicacionToEdit?.usuario_modificador?.nombre ? (
                      <>
                        <p className="text-sm font-black text-slate-800 dark:text-white leading-tight">{ubicacionToEdit.usuario_modificador.nombre}</p>
                        <p className="text-[10px] text-slate-400 font-medium mt-0.5">
                          {mounted && ubicacionToEdit.updated_at ? format(new Date(ubicacionToEdit.updated_at), 'dd MMM, hh:mm aa') : ''}
                        </p>
                      </>
                    ) : (
