...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!body.categorias || !Array.isArray(body.categorias)) {
      return NextResponse.json({ error: 'Se requiere un array "categorias".' }, { status: 400 })
    }
    ...
    for (let i = 0; i < body.categorias.length; i++) {
      const row = body.categorias[i]
      const rowNumber = i + 1
      ...
      try {
        ...
        if (existente) {
          const razon = existente.codigo?.toLowerCase() === parsed.codigo.toLowerCase()
            ? `Ya existe una categoría con el código "${parsed.codigo}".`
            : `Ya existe una categoría con la descripción "${parsed.descripcion}".`
          results.errors.push({ row: rowNumber, codigo: parsed.codigo, descripcion: parsed.descripcion, error: razon })
          continue
        }

        await prisma.materialCategoria.create({
          data: {
