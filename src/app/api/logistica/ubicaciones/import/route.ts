...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!body.ubicaciones || !Array.isArray(body.ubicaciones)) {
      return NextResponse.json({ error: 'Se requiere un array "ubicaciones".' }, { status: 400 })
    }
    ...
    for (let i = 0; i < body.ubicaciones.length; i++) {
      const row = body.ubicaciones[i]
      const rowNumber = i + 1
      ...
      try {
        ...
        if (existente) {
          const razon = existente.codigo?.toLowerCase() === parsed.codigo.toLowerCase()
            ? `Ya existe una ubicación con el código "${parsed.codigo}".`
            : `Ya existe una ubicación con la descripción "${parsed.descripcion}".`
