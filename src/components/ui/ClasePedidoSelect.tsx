...
interface ClasePedidoSelectProps {
  onSelect: (clase: ClasePedido) => void
  placeholder?: string
  className?: string
  selectedLabel?: string
}

export default function ClasePedidoSelect({ onSelect, placeholder = 'Seleccionar clase...', className, selectedLabel }: ClasePedidoSelectProps) {
  const [open, setOpen] = useState(false)
  const [search, setSearch] = useState('')
  const [options, setOptions] = useState<ClasePedido[]>([])
  ...
  useEffect(() => {
    const fetchClases = async () => {
      ...
      try {
        ...
        console.error('Error searching clases:', error)
      } finally {
        setLoading(false)
      }
    }

    const timer = setTimeout(fetchClases, 300)
  ...
  useEffect(() => {
    function handleClickOutside(event: MouseEvent) {
      if (containerRef.current && !containerRef.current.contains(event.target as Node)) {
        const portalDropdown = document.getElementById('clase-pedido-select-portal')
    ...
    <div className={cn("relative", className)} ref={containerRef}>
      ...
      >
        ...
        <span className="material-symbols-outlined text-slate-400 group-hover:text-blue-500 transition-colors">
          {open ? 'expand_less' : 'category'}
        </span>
      </div>

      {open && mounted && createPortal(
        <div 
          id="clase-pedido-select-portal"
          style={{ 
            position: 'absolute',
            top: coords.top + 8,
            left: coords.left,
          ...
          <div className="max-h-64 overflow-y-auto py-2">
            {loading ? (
              ...
              options.map((c) => (
                ...
                >
                  <div className="flex flex-col">
                    <span className="text-[10px] font-black text-blue-500 group-hover:text-blue-200 mb-0.5 uppercase tracking-tighter">
                      {c.codigo}
                    </span>
              ...
              <div className="px-4 py-12 text-center text-slate-400">
                ...
                <p className="text-[10px] uppercase font-black tracking-widest">No se encontraron clases</p>
              </div>
            )}
          </div>
        </div>,
